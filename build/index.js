...
// 색상 팔레트 표시
function displayColorPalette(colorSet) {
    if (!colorSet)
        return "색상 정보 없음";
...
// 이미지 및 JSON 저장 함수
function saveDataToFiles(nftData, outputDir = "nft-output") {
...
// MCP 도구 등록
server.tool("get-nft-data", "Get basic NFT information", {}, async () => {
    const apiUrl = `${BASE_URL}${SUBPATH}`;
    const nftData = await makeApiRequest(apiUrl);
    if (!nftData) {
        return {
            content: [
                {
...
server.tool("get-nft-image", "Get NFT image data", {}, async () => {
    const apiUrl = `${BASE_URL}${SUBPATH}`;
    const nftData = await makeApiRequest(apiUrl);
    if (!nftData) {
        return {
...
server.tool("get-nft-attributes", "Get detailed NFT attributes", {}, async () => {
    const apiUrl = `${BASE_URL}${SUBPATH}`;
    const nftData = await makeApiRequest(apiUrl);
    if (!nftData) {
        return {
    ...
    const attributesText = nftData.data.attributes.map(attr => `${attr.trait_type}: ${attr.value}`).join('\n');
    return {
        content: [
            {
                type: "text",
...
server.tool("get-color-palette", "Get NFT color palette", {}, async () => {
    const apiUrl = `${BASE_URL}${SUBPATH}`;
    const nftData = await makeApiRequest(apiUrl);
    if (!nftData) {
        return {
...
server.tool("get-enhanced-nft-view", "Get enhanced NFT view with formatted display", {}, async () => {
    const apiUrl = `${BASE_URL}${SUBPATH}`;
    const nftData = await makeApiRequest(apiUrl);
    if (!nftData) {
        return {
...
server.tool("save-nft-files", "Save NFT image and JSON data to files", {
...
server.tool("get-random-nfts", "Get multiple random NFTs", {
